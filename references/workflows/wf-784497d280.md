...
工作流路径：`视频换人/视频换人-Animate（自动遮罩）.json`  
核对：2026-09-20｜Wan / Animate / VACE / InfiniteTalk｜按字段填写  
源文件 SHA-256：`d1666fdf1d67a3ab86502d9016527de8afb5979ddbc4b5c0e9cac680aa01470b`
...
| 位置 | 素材 | 说明 |
|---|---|---|
| #57 LoadImage → `image` | 图片 | 实际连接标签：替换人物图像；替换素材后核对预览 |
| #63 VHS_LoadVideo → `video` | 视频 | 实际连接标签：视频转图像、帧计数、音频、视频信息；替换素材后核对预览 |
...
1. 在工作流列表打开 `视频换人/视频换人-Animate（自动遮罩）.json`，确认名称和主要节点与本卡一致；缺节点时先处理加载问题。
...
选区处理链定位：#100 GrowMask；#197 LayerMask: SegformerB2ClothesUltra；#108 BlockifyMask；#99 DrawMaskOnImage；#120 FaceMaskFromPoseKeypoints。

## 获取结果与验收

结果位置：#246 VHS_VideoCombine；#255 VHS_VideoCombine；#256 VHS_VideoCombine；#231 VHS_VideoCombine；#189 VHS_VideoCombine。在对应预览保存/下载，核对本次输出时间。
